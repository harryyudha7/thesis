...
F_BOUNDARY = [0.0, 2]  # boundary condition value
eps = 0.01
...
def make_loss_fn(f: Callable, dfdx: Callable, d2fdx2: Callable) -> Callable:
    ...
    def loss_fn(params: torch.Tensor, x: torch.Tensor):
        ...
        # interior loss
        f_value = f(x, params)
        # interior = (1-2*x)*dfdx(x,params) - 2*f_value
        # interior = dfdx(x, params) + f_value
        # interior = d2fdx2(x,params)/10 + dfdx(x, params)
        ...
        # x_boundary = torch.tensor([x0[0]])
        # f_boundary = torch.tensor([f0[0]])
        ...
        ) #+ 1/(torch.sum(f_value)+0.1)
        # print(1/(torch.sum(f_value)+0.1))
        return loss_value
...
if __name__ == "__main__":
    ...
    # analytical_sol_fn = lambda x: np.exp(1-x)
    # analytical_sol_fn = lambda x: -np.exp(1-x/eps)+np.exp(1)
